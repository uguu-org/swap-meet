...
if [[ $# -ne 1 ]]; then
   echo "$0 {generate_table_from_grayscale_image.exe}"
   exit 1
fi
TOOL=$1
...
0 0 0 0
EOT
perl -e 'print "0 255 0 0\n" x 31' >> "$TEST_DIR/expected_pixels.pgm"
perl -e 'print "0 255 255 0\n" x 223' >> "$TEST_DIR/expected_pixels.pgm"
echo "0 255 255 255" >> "$TEST_DIR/expected_pixels.pgm"
pnmtopng \
